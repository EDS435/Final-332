...
class GirdView extends React.Component {
          ...
          <Card  style={{ marginBottom: '20px' ,marginTop: '20px'}}>
            <Card.Body>
              ...
              <Link className={this.isActiveTab('/account/:id')} to={"/account/"+ char._id} onClick={event => this.onTabClick(event, '/account/:id')}>
                <button style={{  float: 'right', padding: '12px', margin:'10px', color: 'white', borderRadius: '35px', background:'green'}} type="button" >View Account</button>
              </Link>
              <button type='button'
  ...
  render(){
    ...
    return(
    <div>
      <h3 style={{textAlign:'center', fontSize: '50px', margin: '10px',  color: 'black', fontWeight: 'bold'}}>Welcome Dear Travelers</h3>
